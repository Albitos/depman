# Why depman?
There are two reasons for using depman:
...
It automatically adds an alias for those programs and executes them through configured docker container.
This isolates all scripts executed by packages to the local directory and prevents malicious code from accessing the developer's machine.
...
Use `composer`, `pip` or `npm` as usual. They will be executed from within a docker container using version defined in config.

You can switch docker image and tag using the following command:

`depman_switch_image [plugin]`

It will set an image and tag for the current directory and save it to a config file.
